...
/**
 * Configure in-session and optional persisted command history.
 *
 * @param {Object} [options]
 * @param {number} [options.sessionMaxEntries=100]
 * @param {boolean} [options.persist=false]
 * @param {string} [options.path] - default ~/.node-menu_history when persist is true
 * @param {number} [options.maxEntries=100] - persisted list max
 * @returns {NodeMenu} for chaining calls
 */
NodeMenu.prototype.configureHistory = function(options) {
    var self = this;
    options = options || {};
    self.historyOptions = {
        sessionMaxEntries: options.sessionMaxEntries != null ? options.sessionMaxEntries : 100,
        persist: !!options.persist,
        path: options.path,
        maxEntries: options.maxEntries != null ? options.maxEntries : 100
    };
    return self;
};
...
NodeMenu.prototype.start = function() {
    ...
    self._history = new History({
        sessionMaxEntries: self.historyOptions.sessionMaxEntries,
        persist: self.historyOptions.persist,
        path: self.historyOptions.path,
        maxEntries: self.historyOptions.maxEntries
    });

    if (self._history.persist) {
        self._history.load();
        self._history.seedSessionFromPersisted();
    }
    ...
    self._rl = readline.createInterface({
        ...
        historySize: self.historyOptions.sessionMaxEntries
    });
    self._rl.history = self._history.getSessionForReadline();

    function syncReadlineHistory() {
        self._rl.history = self._history.getSessionForReadline();
    }

    function ask() {
        process.stdout.write('\n');
        self._rl.question('>> ', handleLine);
    }
    ...
    function handleLine(text) {
        ...
        self._history.recordSession(line);
        syncReadlineHistory();

        var args = self._parseInput(line);
        var item = null;
        ...
        if (!item) {
            self.consoleOutput('Command not found: ' + line);
        } else {
            ...
            if (!valid.lengthMatch) {
                ...
                var persistedBefore = self._history.getPersisted();
                self._history.recordPersisted(line);
                try {
                    item.handler.apply(item.owner, args.argv);
                } catch (err) {
                    self._history.replacePersisted(persistedBefore);
                    self.consoleOutput('Error: ' + (err && err.message ? err.message : err));
                }
            }
        }
